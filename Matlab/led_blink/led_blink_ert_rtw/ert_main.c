...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) led_blink_M->Timing.clockTick0;
  led_blink_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(led_blink_M->extModeInfo, &rtmGetTFinal(led_blink_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(led_blink_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(led_blink_M);
