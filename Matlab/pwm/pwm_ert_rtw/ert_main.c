...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) pwm_M->Timing.clockTick0;
  pwm_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(pwm_M->extModeInfo, &rtmGetTFinal(pwm_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(pwm_M, true);
    }
  }
